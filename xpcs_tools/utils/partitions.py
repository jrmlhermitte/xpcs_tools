...
def partition2D(qlist,philist,qvals,phivals,whole=False):
    ''' Partition a list of pixels with values qvals and phivals
        ...
        If no elements, returns -1,-1, [] (empty list)
            for qid, pid pselect

        whole : decide whether or not to return whole list
    '''
    qvals = np.array(qvals).reshape(-1)
    phivals = np.array(phivals).reshape(-1)
    qid = np.searchsorted(qlist.ravel(),qvals,"right")
    pid = np.searchsorted(philist.ravel(),phivals,"right")
    nopselect = np.where((qid %2 == 0) + (pid %2 == 0))[0]
    pselect = np.where((qid % 2 == 1)*(pid %2 == 1))[0]

    #set entries that are not in list to -1
    if len(nopselect) > 0:
        # will be divided by 2 later so set to 2 first, then will become -1
        qid[nopselect] = -2
        pid[nopselect] = -2

    if not whole:
        if len(pselect) == 0:
            print("Warning, no pixels found in partition")
            return -1, -1, []
...
def partition3D(xlist,ylist, zlist, xvals,yvals, zvals,whole=False):
    ''' Partition a list of pixels with values xvals, yvals, zvals
        xlist, ylist and zlist
        Lists are a list of widths [start1,finish1, start2,finish2] etc.
        Note this can be generalized to any 2D coordinate system.
        ...
        whole : decide whether or not to return whole list
    '''
    xvals = np.array(xvals).reshape(-1)
    yvals = np.array(yvals).reshape(-1)
    zvals = np.array(zvals).reshape(-1)
    xlist= np.searchsorted(xlist.ravel(),qvals,"right")
    ylist= np.searchsorted(ylist.ravel(),qvals,"right")
    zlist= np.searchsorted(zlist.ravel(),qvals,"right")
    pid = np.searchsorted(philist.ravel(),phivals,"right")
    nopselect = np.where((qid %2 == 0) + (pid %2 == 0))[0]
    pselect = np.where((qid % 2 == 1)*(pid %2 == 1))[0]
