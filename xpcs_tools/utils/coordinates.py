import numpy as np
from xpcs_tools.utils.partitions import partition2D, partition_avg, linplist
...
def mkqphi(img, rdet, dpix, wv, x0, y0):
    ''' Make qphi coordinates for a 2D image, 
            taking Ewald curvature into account.

        Parameters
        ----------

        img : the image you want to make coordinates for
        rdet : sample to detector distance (m)
        dpix : pixel transverse size (m)
        wv : wavelength (Angs)
        x0,y0 : beam center (pixels)

            '''
    x, y = np.arange(img.shape[1]).astype(float), np.arange(img.shape[0]).astype(float)
    X, Y = np.meshgrid(x,y)
    X -= x0
    Y -= y0
    R = np.sqrt(X**2 + Y**2)*dpix

    tth = np.arctan2(R, rdet)
    Q = 4*np.pi/wv*np.sin(tth/2.)
    PHI = np.arctan2(Y,X)

    return Q, PHI

def mkqxyz(img, rdet, dpix, wv, x0, y0, detector_tilt=0):
    ''' Make qphi coordinates for a 2D image, 
            taking Ewald curvature into account.
        ...
        wv : wavelength (Angs)
        x0,y0 : beam center (pixels)
        detector_tilt : detector tilt in degrees
    ...
    # rotate about X
    Xp = X*np.cos(detector_tilt)
    Xpp = X*np.sin(detector_tilt)

    #tth = np.arctan2(R, rdet)
    # rdet to X changes due to tilt
    rdetp = rdet - Xpp
    # the r vector also changes with tilt
    Rp = np.sqrt(Xp**2 + Y**2)
    
    tth = np.arctan2(Rp,rdetp)

    kmag = 2*np.pi/wv

    PHI = np.arctan2(Y,Xp)
    QX = kmag*np.sin(tth)*np.cos(PHI)
    QY = kmag*np.sin(tth)*np.sin(PHI)
    QZ = kmag*(1-np.cos(tth))
